...
const opportunityDirectoryEntityId = "b62c6fad-c5a8-f111-9b33-6045bda94b17"; // CustomerOpportunityDirectory
...
const decodeJwtEmail = (token: string | null | undefined): string => {
...
type SoldGranularity = "day" | "month" | "quarter" | "year";
...
const soldBucketKey = (isoDate: string, granularity: SoldGranularity) => {
  switch (granularity) {
    case "day":
      return isoDate;
    case "month":
...
type SoldPoint = { bucket: string; sold: number };
...
const latestPurchasedByCustomer = (
  rows: BurndownRow[],
        ...
        <table className="condensed-table no-scroll-table">
          ...
          <tbody>
            {rows.map(({ rollup, latest }) => (
              <tr key={rollup.Id}>
                <td className="customer">
                  <button className="customer-link" onClick={() => onSelectCustomer(rollup.CustomerId)}>
                    {rollup.CustomerName}
                  </button>
                </td>
...
function LicensePortal({
  ...
  const [view, setView] = useState<"home" | "contracts" | "sold" | "utilization">("home");
